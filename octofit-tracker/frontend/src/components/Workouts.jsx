...
function Workouts() {
  ...
  useEffect(() => { fetch(workoutsEndpoint).then((response) => { if (!response.ok) throw new Error('Could not load workouts'); return response.json() }).then((payload) => setWorkouts(collectionFrom(payload))).catch((reason) => setError(reason.message)) }, [])
